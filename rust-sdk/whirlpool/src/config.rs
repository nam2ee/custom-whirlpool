use std::{error::Error, sync::{Mutex, RwLock}};
...
pub static WHIRLPOOLS_CONFIG_ADDRESS: RwLock<Pubkey> =
    RwLock::new(SOLANA_MAINNET_WHIRLPOOLS_CONFIG_ADDRESS);

/// The currently selected address for the Whirlpools program's config extension account.
pub static WHIRLPOOLS_CONFIG_EXTENSION_ADDRESS: RwLock<Pubkey> =
    RwLock::new(SOLANA_MAINNET_WHIRLPOOLS_CONFIG_EXTENSION_ADDRESS);
...
pub fn set_whirlpools_config_address(input: WhirlpoolsConfigInput) -> Result<(), Box<dyn Error>> {
    ...
    *WHIRLPOOLS_CONFIG_ADDRESS.write()? = address;
    *WHIRLPOOLS_CONFIG_EXTENSION_ADDRESS.write()? =
...
pub fn reset_configuration() -> Result<(), Box<dyn Error>> {
    *WHIRLPOOLS_CONFIG_ADDRESS.write()? = SOLANA_MAINNET_WHIRLPOOLS_CONFIG_ADDRESS;
    *WHIRLPOOLS_CONFIG_EXTENSION_ADDRESS.write()? =
