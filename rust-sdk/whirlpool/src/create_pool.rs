...
pub async fn create_concentrated_liquidity_pool_instructions(
    ...
    let pool_address = get_whirlpool_address(
        &*WHIRLPOOLS_CONFIG_ADDRESS.read()?,
        &token_a,
        &token_b,
        tick_spacing,
    )?
    ...
    let fee_tier = get_fee_tier_address(&*WHIRLPOOLS_CONFIG_ADDRESS.read()?, tick_spacing)?.0;

    let token_badge_a =
        get_token_badge_address(&*WHIRLPOOLS_CONFIG_ADDRESS.read()?, &token_a)?.0;

    let token_badge_b =
        get_token_badge_address(&*WHIRLPOOLS_CONFIG_ADDRESS.read()?, &token_b)?.0;
    ...
    instructions.push(
        InitializePoolV2 {
            whirlpools_config: *WHIRLPOOLS_CONFIG_ADDRESS.read()?,
